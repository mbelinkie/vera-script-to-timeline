...
| 21.1.0 / bundle build 21.1.00014 | macOS 15.1 x86_64 | Real API connection reported `DaVinci Resolve Studio`, version 21.1.0, build 14, empty suffix | Available through the supported local external scripting API | Audit project `VERA EV24 Capability 20260922-030002` created four V4-at-frame-0 lower-thirds from the pinned EV24 setting: standard automatic fill (120), explicit Top/Bottom override (120), Otis without a semantic Year (64), and the declared package test badge (64). The project saved, closed, reopened, and its four V4 items retained their exact durations, one Fusion composition, the unique `EV24LowerThird` `MacroOperator`, and every applicable control value. | `GetToolList(False)` returns the expanded graph as numeric-keyed entries (68 in the observed import), rather than a name-keyed one-entry macro map. The safe discovery rule is therefore the unique `TOOLS_Name=EV24LowerThird` and `TOOLS_RegID=MacroOperator`, plus read/write checks for all five registered inputs; missing, renamed, duplicate, or incompatible identities stop the run. The initial failed project `VERA EV24 Capability 20260922-030001` is retained as evidence of that corrected observation. | Retained result: `docs/verification/issue-3/ev24-capability-20260922-030002.json`; first-run limitation record: `docs/verification/issue-3/ev24-capability-20260922-030001.json`; connected run observed 2026-09-22 | Producer visual acceptance of the 64- and 120-frame animation is pending. |
