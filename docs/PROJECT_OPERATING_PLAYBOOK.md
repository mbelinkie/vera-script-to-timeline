...
| Read-only board dashboard | Generate a producer-friendly view of completed, active, blocked, and future work from the live Project rather than maintaining a second status spreadsheet or markdown tracker. Provide a one-click launcher as well as the command-line refresh. The dashboard is safe to refresh because it never mutates either Project. | `npm run progress`; [Open VERA Progress.command](../Open%20VERA%20Progress.command); [dashboard plan](plans/board-driven-progress-dashboard.md) |
...
| Astra reconsideration register | After hard work closes, capture only evidence-backed unresolved engineering risks in a short register: observed failure, precise later-review question, and retirement condition. It is a decision aid, not a shadow roadmap or automatic routing rule. | [Possible Issues for Astra](POSSIBLE_ISSUES_FOR_ASTRA.md) |
