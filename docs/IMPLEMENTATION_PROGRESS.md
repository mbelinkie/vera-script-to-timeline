...
| 0.3 Resolve Free import trial | In progress | Producer + agent preparer/recorder | 0.2 accepted | Automated preparation is CI-green. Producer began real Free testing and observed that the FCPXML import omitted the expected marker; the OTIO result, remaining comparison fields, and D-P005 remain required. |
...
| 2026-08-24 | Producer | Begin Slice 0.3 imports in actual Resolve Free and record observed behavior | In progress | Producer reported Resolve Free 21 on an Intel Mac, installed from the Blackmagic download, and observed that the FCPXML import omitted the expected marker. Other FCPXML fields, the OTIO result, evidence locators, and the maintain-or-park decision remain unrecorded. |
