...
| D-0014 | Accepted | Use the bounded Slice 1.3 compiler decisions and contract change in `docs/plans/slice-1.3-contract-change.md` and `docs/plans/slice-1.3-compiler-core.md`: add `CompilerDependencies v1`; add honest derived-end and unavailable timing labels to `TimelineManifest v1`; use exact rational ceiling frame boundaries; reject duplicate active order keys and unusable timing; preserve failed narration duration while blocking its anchored visuals; and use the documented adjustable track/cut/report rules. | Approved explicitly by the producer on 2026-08-26. Slice 1.3 is the first TypeScript consumer of Python-produced narration timing, meeting Slice 1.2's explicit contract-change trigger. The approved boundary avoids promoting the full cache record and prevents bare `word` from overstating provider evidence whose word ends are derived. |
