...
| D-0009 | Superseded by D-0012 | Use AWS Polly Neural as Slice 1.2's initial cloud `SpeechSynthesisProvider`, initially proposing profile `aws-polly-joanna-neural-en-us-v1` (`Joanna`, `neural`, `en-US`, default `us-east-1`). Preserve word/sentence speech marks, record `voiceVersion: provider_not_supplied`, and cache result bytes immutably rather than pretending the managed voice has a frozen vendor revision. | The provider/engine choice remains binding. The producer's pre-acceptance voice judgment selected Matthew as the default and retained Joanna as an alternative, recorded by D-0012. |
...
| D-0012 | Accepted | Keep AWS Polly Neural and make `aws-polly-matthew-neural-en-us-v1` (`Matthew`) the default named voice profile, with `aws-polly-joanna-neural-en-us-v1` (`Joanna`) as an explicitly selectable alternative. Slice 1.2 exposes the choice through `--voice-profile`; the Phase 2 app will persist a project-level selection and expose it in the build interface. | The producer selected Matthew on 2026-08-25 before live Slice 1.2 acceptance and requested Joanna remain available. Both voices support Neural synthesis and speech marks in `us-east-1`. Voice/profile identity is already part of the immutable synthesis key, so changing profiles cannot relabel an older asset. This supersedes only D-0009's proposed default voice; its provider, engine, provenance, timing, and cache decisions remain unchanged. |
| D-0013 | Accepted | Ship the producer-authored, hash-pinned `Text+` `.drb` and place it through `ImportFolderFromFile` plus `AppendToTimeline` instead of `InsertFusionTitleIntoTimeline`. Support only `Text+`; resolve destination track IDs through the adjustable manifest, use the validated generator end-frame rule, retain the imported bin for provenance, and fail closed without a stock-insertion fallback. | Resolve Studio 21.0.4 build 5 validation proved the pinned generator has the same public Fusion registration-ID fingerprint as stock Text+ (`MediaOut`, `TextPlus`), that `endFrame = requested duration + 1` produces exact 24- and 72-frame occurrences, and that V4/start/duration survive save/reopen. Integrated project `VERA TextPlus Integrated Acceptance 20260825-234847` then verified exact V4/72-frame placement and event accounting after reopen; the producer visually accepted it on 2026-08-25. This resolves destination-track and duration control only for the shipped pinned-template path. Stock-title catalog enumeration and arbitrary Fusion titles remain open/out of scope. |
| D-0014 | Pending producer approval | Approve the bounded Slice 1.3 compiler decisions and contract change in `docs/plans/slice-1.3-contract-change.md` and `docs/plans/slice-1.3-compiler-core.md`: add `CompilerDependencies v1`; add honest derived-end timing labels to `TimelineManifest v1`; use exact rational ceiling frame boundaries; reject duplicate active order keys and unusable timing; preserve failed narration duration while blocking its anchored visuals; and use the documented adjustable track/cut/report rules. | Slice 1.3 is the first TypeScript consumer of Python-produced narration timing. Slice 1.2 explicitly requires a producer-approved contract note before that record crosses languages. The current manifest's bare `word` precision would also overstate provider evidence whose word ends are derived. Implementation remains paused until this one bundled decision is approved. |
